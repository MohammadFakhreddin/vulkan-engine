...
inline static constexpr const char * DebugLayer = "VK_LAYER_LUNARG_standard_validation";
struct State {
  //
  ...
  VkSwapchainKHR old_swap_chain = nullptr;
  ...
  /* std::vector<VkBuffer> uniforbuffers;
  std::vector<VkDeviceMemory> uniforbuffers_memory;*/
...
VkExtent2D ChooseSwapExtent(const VkSurfaceCapabilitiesKHR& surface_capabilities) {
    if (surface_capabilities.currentExtent.width <= 0) {
        VkExtent2D const swap_chain_extent = {
            MathHelper::Min(
                MathHelper::Max(m_state.screen_width, surface_capabilities.minImageExtent.width), 
                surface_capabilities.maxImageExtent.width
            ),
            MathHelper::Min(
                MathHelper::Max(m_state.screen_height, surface_capabilities.minImageExtent.height), 
...
bool Init(InitParams const & params) {
  {
    m_state.screen_width = params.screen_width;
    m_state.screen_height = params.screen_height;
  }
  create_window();
  create_instance();
...
void CreateBuffer(
  ...
  assert(nullptr != m_state.device);
  assert(nullptr != m_state.physical_device);
  ...
  if (vkCreateBuffer(*m_state.device, &buffer_info, nullptr, &out_buffer) != VK_SUCCESS) {
    throw std::runtime_error("Application::createBuffer::vkCreateBuffer failed to create buffer!");
  }

  VkMemoryRequirements memory_requirements;
  vkGetBufferMemoryRequirements(*m_state.device, out_buffer, &memory_requirements);
  ...
  if (vkAllocateMemory(*m_state.device, &memory_allocation_info, nullptr, &out_buffer_memory) != VK_SUCCESS) {
    throw std::runtime_error("Application::createBuffer::vkAllocateMemory failed to allocate buffer memory!");
  }

  if(VK_SUCCESS != vkBindBufferMemory(*m_state.device, out_buffer, out_buffer_memory, 0)) {
...
bool CreateTextureImage(
  ...
  if(cpu_texture.isValid()) {
    ...
    void * data = nullptr;
    vkMapMemory(*m_state.device, staging_buffer_memory, 0, image_size, 0, &data);
    assert(nullptr != data);
    ::memcpy(data, pixels, static_cast<size_t>(image_size));
    vkUnmapMemory(*m_state.device, staging_buffer_memory);
    ...
    vkDestroyBuffer(*m_state.device, staging_buffer, nullptr);

    vkFreeMemory(*m_state.device, staging_buffer_memory, nullptr);
...
void DestroyTextureImage(
  ...
  vkDestroyImage(*m_state.device, *texture_image, nullptr);
  vkFreeMemory(*m_state.device, *texture_memory, nullptr);
}
...
VkSampler CreateTextureSampler() {
  ...
  if (vkCreateSampler(*m_state.device, &sampler_info, nullptr, &sampler) != VK_SUCCESS) {
...
static void DestoryBuffer(VkBuffer * buffer, VkDeviceMemory * memory) {
  vkDestroyBuffer(*m_state.device, *buffer, nullptr);
  vkFreeMemory(*m_state.device, *memory, nullptr);
}
...
static void create_window() {
    ...
    assert(screen_info.valid);
    
    m_state.window = SDL_CreateWindow(
        "VULKAN_ENGINE", 
        uint32_t((screen_info.screen_width / 2.0f) - (m_state.screen_width / 2.0f)), 
        uint32_t((screen_info.screen_height / 2.0f) - (m_state.screen_height / 2.0f)),
        m_state.screen_width, m_state.screen_height,
        SDL_WINDOW_SHOWN /*| SDL_WINDOW_FULLSCREEN */| SDL_WINDOW_VULKAN
    );
    assert(nullptr != m_state.window);
}
...
static void create_instance() {
    ...
    {// Filling sdl extensions
        ...
        SDL_Check(SDL_Vulkan_GetInstanceExtensions(m_state.window, &sdl_extenstion_count, nullptr));
    ...
    VK_Check(vkCreateInstance(&instanceInfo, NULL, m_state.vk_instance));
}

static void create_debug_callback() {
    ...
    VK_Check(debug_report_callback(
        *m_state.vk_instance, 
        &debug_info, 
        nullptr, 
        &m_state.vk_debug_report_callback_ext
    ));
}
...
static void create_window_surface() {
    ...
    SDL_Vulkan_CreateSurface(
        m_state.window,
        *m_state.vk_instance,
        &m_state.window_surface
    );
}

static void find_physical_device(uint8_t const retry_count) {
    ...
    m_state.physical_device = devices[retry_count];
    m_state.device_features.samplerAnisotropy = VK_TRUE;
    m_state.device_features.shaderClipDistance = VK_TRUE;
    m_state.device_features.shaderCullDistance = VK_TRUE;

    VkPhysicalDeviceProperties deviceProperties;
    vkGetPhysicalDeviceProperties(m_state.physical_device, &deviceProperties);
    vkGetPhysicalDeviceFeatures(m_state.physical_device, &m_state.device_features);
...
static bool find_queue_families() {
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(m_state.physical_device , &queue_family_count, queueFamilies.data());
    ...
    for (uint32_t i = 0; i < queue_family_count; i++) {
        ...
        vkGetPhysicalDeviceSurfaceSupportKHR(m_state.physical_device , i, m_state.window_surface, &present_is_supported);
        if (queueFamilies[i].queueCount > 0 && queueFamilies[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) {
            m_state.graphics_queue_family = i;
            found_graphic_queue_family = true;
            if (present_is_supported) {
                m_state.present_queue_family = i;
    ...
    if (found_graphic_queue_family) {
        std::cout << "queue family #" << m_state.graphics_queue_family << " supports graphics" << std::endl;

        if (found_present_queue_family) {
            std::cout << "queue family #" << m_state.present_queue_family << " supports presentation" << std::endl;
            ret = true;
        } else {
...
static void create_logical_device() {
    ...
    queue_create_info[0].queueFamilyIndex = m_state.graphics_queue_family;
    ...
    if (m_state.graphics_queue_family == m_state.present_queue_family) {
    ...
    deviceCreateInfo.pEnabledFeatures = &m_state.device_features;

    deviceCreateInfo.enabledLayerCount = 1;
    deviceCreateInfo.ppEnabledLayerNames = &DebugLayer;

    if (vkCreateDevice(m_state.physical_device, &deviceCreateInfo, nullptr, m_state.device) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create logical device");
    }

    std::cout << "created logical device" << std::endl;
    // Get graphics and presentation queues (which may be the same)
    vkGetDeviceQueue(*m_state.device, m_state.graphics_queue_family, 0, &m_state.graphic_queue);
    vkGetDeviceQueue(*m_state.device, m_state.present_queue_family, 0, &m_state.present_queue);

    std::cout << "acquired graphics and presentation queues" << std::endl;
    vkGetPhysicalDeviceMemoryProperties(m_state.physical_device, &m_state.device_memory_properties);
}
...
static void create_command_pool() {
    ...
    pool_create_info.queueFamilyIndex = m_state.graphics_queue_family;

    if (vkCreateCommandPool(*m_state.device, &pool_create_info, nullptr, &m_state.command_pool) != VK_SUCCESS) {
...
static uint32_t findMemoryType (
    ...
    vkGetPhysicalDeviceMemoryProperties(m_state.physical_device, &memProperties);
...
static bool create_image_view(
    ...
    if (vkCreateImageView(*m_state.device, &createInfo, nullptr, out_image_view) != VK_SUCCESS) {
        ret = false;
    }
    return ret;
}
...
static void create_swap_chain_image_views() {
    m_state.swap_chain_image_views.resize(m_state.swap_chain_images.size());

    // Create an image view for every image in the swap chain
    for (size_t i = 0; i < m_state.swap_chain_images.size(); i++) {
        create_image_view(
            &m_state.swap_chain_image_views[i],
            m_state.swap_chain_images[i], 
            m_state.swap_chain_format,
    ...
    create_image(
        m_state.depth_image, 
        m_state.depth_image_memory,
        m_state.swap_chain_extent.width, 
        m_state.swap_chain_extent.height, 
        depth_format, 
        VK_IMAGE_TILING_OPTIMAL, 
        VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, 
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
    );
    create_image_view(&m_state.depth_image_view, m_state.depth_image, depth_format, VK_IMAGE_ASPECT_DEPTH_BIT);
...
static void create_image(
    ...
    if (vkAllocateMemory(*m_state.device, &allocate_info, nullptr, &out_image_memory) != VK_SUCCESS) {
        throw std::runtime_error("Failed to allocate image memory!");
    }

    vkBindImageMemory(*m_state.device, out_image, out_image_memory, 0);
}
...
static VkCommandBuffer begin_single_time_commands() {
    ...
    allocate_info.commandPool = m_state.command_pool;
    allocate_info.commandBufferCount = 1;

    VkCommandBuffer commandBuffer;
    vkAllocateCommandBuffers(*m_state.device, &allocate_info, &commandBuffer);
...
static void end_single_time_commands(VkCommandBuffer command_buffer) {
    ...
    vkQueueSubmit(m_state.graphic_queue, 1, &submit_info, VK_NULL_HANDLE);
    vkQueueWaitIdle(m_state.graphic_queue);

    vkFreeCommandBuffers(*m_state.device, m_state.command_pool, 1, &command_buffer);
}
...
static VkFormat find_supported_format (
    ...
    for (VkFormat format : candidates) {
        ...
        vkGetPhysicalDeviceFormatProperties(m_state.physical_device, format, &props);
...
static void create_swap_chain() {
    ...
    if (vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_state.physical_device, m_state.window_surface, &surface_capabilities) != VK_SUCCESS) {
        throw std::runtime_error("Failed to acquire presentation surface capabilities");
    }

    // Find supported surface formats
    uint32_t formatCount;
    if (vkGetPhysicalDeviceSurfaceFormatsKHR(m_state.physical_device, m_state.window_surface, &formatCount, nullptr) != VK_SUCCESS || formatCount == 0) {
    ...
    if (vkGetPhysicalDeviceSurfaceFormatsKHR(m_state.physical_device, m_state.window_surface, &formatCount, surface_formats.data()) != VK_SUCCESS) {
        throw std::runtime_error("Failed to get supported surface formats");
    }

    // Find supported present modes
    uint32_t presentModeCount;
    if (vkGetPhysicalDeviceSurfacePresentModesKHR(m_state.physical_device, m_state.window_surface, &presentModeCount, nullptr) != VK_SUCCESS || presentModeCount == 0) {
    ...
    if (vkGetPhysicalDeviceSurfacePresentModesKHR(m_state.physical_device, m_state.window_surface, &presentModeCount, present_modes.data()) != VK_SUCCESS) {
    ...
    m_state.swap_chain_extent = ChooseSwapExtent(surface_capabilities);
    ...
    createInfo.surface = m_state.window_surface;
    createInfo.minImageCount = imageCount;
    createInfo.imageFormat = surface_format.format;
    createInfo.imageColorSpace = surface_format.colorSpace;
    createInfo.imageExtent = m_state.swap_chain_extent;
    ...
    createInfo.oldSwapchain = m_state.old_swap_chain;

    if (vkCreateSwapchainKHR(*m_state.device, &createInfo, nullptr, &m_state.swap_chain) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create swap chain");
    }
    std::cout << "created swap chain" << std::endl;
    
    if (m_state.old_swap_chain != nullptr) {
        vkDestroySwapchainKHR(*m_state.device, m_state.old_swap_chain, nullptr);
    }
    m_state.old_swap_chain = m_state.swap_chain;

    m_state.swap_chain_format = surface_format.format;
...
// Store the images used by the swap chain
    ...
    if (
        vkGetSwapchainImagesKHR(*m_state.device, m_state.swap_chain, &actual_image_count, nullptr) != VK_SUCCESS || 
        actual_image_count == 0
    ) {
        throw std::runtime_error("Failed to acquire number of swap chain images");
    }

    m_state.swap_chain_images.resize(actual_image_count);

    if (vkGetSwapchainImagesKHR(
        *m_state.device,
        m_state.swap_chain,
        &actual_image_count,
        m_state.swap_chain_images.data()
    ) != VK_SUCCESS) {
...
static void create_render_pass() {
    ...
    color_attachment.format = m_state.swap_chain_format;
    ...
    if (vkCreateRenderPass(*m_state.device, &createInfo, nullptr, &m_state.render_pass) != VK_SUCCESS) {
...
static void create_frame_buffers() {
    m_state.swap_chain_frame_buffers.resize(m_state.swap_chain_images.size());
    ...
    for (size_t i = 0; i < m_state.swap_chain_images.size(); i++) {
        ...
        std::vector<VkImageView> attachments = {
            m_state.swap_chain_image_views[i],
            m_state.depth_image_view
        };

        VkFramebufferCreateInfo create_info = {};
        create_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
        create_info.renderPass = m_state.render_pass;
        create_info.attachmentCount = static_cast<uint32_t>(attachments.size());
        create_info.pAttachments = attachments.data();
        create_info.width = m_state.swap_chain_extent.width;
        create_info.height = m_state.swap_chain_extent.height;
        create_info.layers = 1;

        if (vkCreateFramebuffer(*m_state.device, &create_info, nullptr, &m_state.swap_chain_frame_buffers[i]) != VK_SUCCESS) {
...
static CreateGraphicPipelineResult create_graphic_pipeline(CreateGraphicPipelineParams const & params) {
    ...
    viewport.x = 0.0f;
    viewport.y = 0.0f;
    viewport.width = static_cast<float>(m_state.swap_chain_extent.width);
    viewport.height = static_cast<float>(m_state.swap_chain_extent.height);
    ...
    scissor.offset.x = 0;
    scissor.offset.y = 0;
    scissor.extent.width = m_state.swap_chain_extent.width;
    scissor.extent.height = m_state.swap_chain_extent.height;
    ...
    pipelineCreateInfo.renderPass = m_state.render_pass;
    ...
    // No longer necessary
    vkDestroyShaderModule(*m_state.device, vertex_shader_module, nullptr);
    vkDestroyShaderModule(*m_state.device, fragment_shader_module, nullptr);
}

static void destroy_graphic_pipeline(
    VkDescriptorSetLayout * descriptor_set_layout, 
    VkPipelineLayout * pipeline_layout, 
    VkPipeline * graphic_pipeline
) {
    vkDestroyPipeline(*m_state.device, *graphic_pipeline, nullptr);
    vkDestroyPipelineLayout(*m_state.device, *pipeline_layout, nullptr);
    vkDestroyDescriptorSetLayout(*m_state.device, *descriptor_set_layout, nullptr);
}

static VkShaderModule create_shader_module(char const * filename) {
    auto read_raw_file_result = FileSystem::ReadRaw(filename);
    assert(true == read_raw_file_result.success);

    VkShaderModuleCreateInfo createInfo = {};
    createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    createInfo.codeSize = read_raw_file_result.bytes.size();
    createInfo.pCode = reinterpret_cast<uint32_t*>(read_raw_file_result.bytes.data());
    ...
    bool destroy_uniform_buffer(
        ...
        assert(uniform_buffers.size() == m_state.swap_chain_images.size());
    ...
    VkBool32 find_suitable_memory_type(uint32_t & out_type_index, uint32_t type_bits, VkFlags const properties) {
        for (uint32_t i = 0; i < 32; i++) {
            if ((type_bits & 1) == 1) {
                if ((m_state.device_memory_properties.memoryTypes[i].propertyFlags & properties) == properties) {
                    out_type_index = i;
                    return true;
                }
            }
    ...
    void create_descriptor_pool() {
        ...
        poolSize.descriptorCount = static_cast<uint32_t>(m_state.swap_chain_images.size());;

        VkDescriptorPoolCreateInfo poolInfo = {};
        poolInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
        poolInfo.poolSizeCount = 1;
        poolInfo.pPoolSizes = &poolSize;
        poolInfo.maxSets = static_cast<uint32_t>(m_state.swap_chain_images.size());;

        if (vkCreateDescriptorPool(*m_state.device, &poolInfo, nullptr, &m_state.descriptor_pool) != VK_SUCCESS) {
