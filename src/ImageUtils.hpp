#ifndef IMAGE_UTILS_NAMESPACE
#define IMAGE_UTILS_NAMESPACE
...
Load (Data & out_image_data, char const * path) {
    ...
    YUGA::Asset::RawReader raw_reader {path};
    if(raw_reader.ok()){
        MFA_ASSERT(raw_reader.total_size() > 0);
        out_image_data.asset = MFA_ALLOC_GP(
            SystemName,
            raw_reader.total_size()
        );
        raw_reader.read(out_image_data.asset);
                    ...
                    if('D' == format[0] && 'X' == format[1] && '1' == format[2] && '0' == format[3]){
                        ...
                        out_image_data.dimension = [dx10_header]()-> uint8_t
                        {
                            ...
                            switch (dx10_header->resource_dimension)
                            {
                                ...
                                MFA_NOT_IMPLEMENTED_YET("D3D10_RESOURCE_DIMENSION_BUFFER and D3D10_RESOURCE_DIMENSION_UNKNOWN are not implemented.");
                                ret = 0;
                            };
                            return ret;
                        }();
                        ...
                        {
                            TextureFormat ret = TextureFormat::Invalid;
                            switch (dx10_header->dxgi_format)
                            {
                                ...
                                case DXGI_Format::DXGI_FORMAT_BC5_UNORM:
                                    ret = TextureFormat::BC5_UNorm_Linear_RG;
                                    break;
                                case DXGI_Format::DXGI_FORMAT_BC5_SNORM:
                                    ret = TextureFormat::BC5_SNorm_Linear_RG;
                                    break;
                                case DXGI_Format::DXGI_FORMAT_BC4_UNORM:
                                    ret = TextureFormat::BC4_UNorm_Linear_R;
                                    break;
                                case DXGI_Format::DXGI_FORMAT_BC4_SNORM:
                                    ret = TextureFormat::BC4_SNorm_Linear_R;
                                    break;
                                case DXGI_Format::DXGI_FORMAT_R8G8B8A8_UNORM:
                                    ret = TextureFormat::Raw_UNorm_R8G8B8A8_Linear;
                                    break;
                                case DXGI_Format::DXGI_FORMAT_R8G8B8A8_UNORM_SRGB:
                                    ret = TextureFormat::Raw_UNorm_R8G8B8A8_sRGB;
                                    break;
                                case DXGI_Format::DXGI_FORMAT_B8G8R8A8_UNORM:
                                    ret = TextureFormat::Raw_UNorm_R8G8B8A8_Linear;
                                    break;
                                case DXGI_Format::DXGI_FORMAT_R8G8_B8G8_UNORM:
                                    ret = TextureFormat::Raw_UNorm_R8G8_Linear;
                                    break;
                                case DXGI_Format::DXGI_FORMAT_R8_UNORM:
                                    ret = TextureFormat::Raw_UNorm_R8_Linear;
                                    break;
                                default:
                                    break;
                            };
                        ...
                        if(TextureFormat::Invalid != out_image_data.format) {
                            ...
                            out_image_data.mipmaps = MFA_ALLOC_GP(
                                SystemName,
                                sizeof(Mipmap) * out_image_data.mipmap_count
                            );
                            {// Generating mipmap offset from data
                                auto const mipmap_array = out_image_data.mipmaps.as<Mipmap>();
...
Unload(Data * image_data) {
    ...
    if(MFA_PTR_VALID(image_data) && image_data->valid) {
        ...
        if(MFA_PTR_VALID(image_data->asset.ptr)) {
            MFA_FREE_GP(SystemName, image_data->asset);
            image_data->asset = {};
        }
        if(true == MFA_PTR_VALID(image_data->mipmaps.ptr)) {
            MFA_FREE_GP(SystemName, image_data->mipmaps);
            image_data->mipmaps = {};
        }
        ret = true;
    }
